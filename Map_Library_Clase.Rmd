---
title: \textbf{Mapas a través de Library Tmap en R}
...
De acuerdo a la distribución de la tasa de mortalidad infantil, existen condados que resaltan mayormente este indice de mortalidad en carolina del norte, caso que entre a profundizar más adelante al identificar cuales condados estan presentando este un indice de mortalidad por encima de 40.
...
```
Para este caso la distribución espacial de los datos muestra relación positiva entre la tasa de mortalidad y la de natalidad en los condados de carolina del norte.
...
# Crear el mapa
tm_shape(nc_map) +
  tm_polygons(col = "SID79", palette = viridis(5, direction = -1), title = "Mortalidad Infantil", 
              style = "quantile", n = 5, 
              textNA = "No disponible", colorNA = "grey90",
              border.col = "black", border.alpha = 0.8) +
  tm_borders(col = "black", lwd = 1) +
  tm_layout(main.title = "Mortalidad Infantil en Carolina del Norte",
            main.title.size = 1.5,
...
Los condados que presentan mayor tasa de mortalidad infantil son cumberland, meckenlenburg, Guilford y wake por encima de 30000
...
# Modelo lineal con la transformación logarítmica del PIB per cápita
...
Dado que inicialmente el producto percapita y la expectativa de vida no poseian una relación lineal, a traves de la transformación logaritmica de un modelo lineal se logro reducir la variabilida de los mismo e identifcar la tendencia que existe entre ellos, y para ello se puede evidenciar una relación signifcativa para ambos, entre mayor aumenta el PBI la expectativa de vida aumento.
